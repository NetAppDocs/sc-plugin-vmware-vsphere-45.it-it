...
+
Per informazioni su come gli amministratori assegnano le risorse agli utenti, vedere la  https://docs.netapp.com/us-en/snapcenter/get-started/rbac-snapcenter.html["Informazioni SnapCenter sull'utilizzo del controllo degli accessi basato sui ruoli"^].



* Se si desidera eseguire backup coerenti con le macchine virtuali, è necessario che VMware Tools sia installato e in esecuzione. VMware Tools è necessario per interrompere le macchine virtuali.
...
NOTE: Questa opzione è supportata per i datastore nei volumi FlexGroup a partire dal plug-in SnapCenter per VMware vSphere 4.5. Questa opzione non è supportata per gli archivi dati nei volumi FlexGroup nel plug-in SnapCenter per VMware vSphere 4.4 e versioni precedenti.
