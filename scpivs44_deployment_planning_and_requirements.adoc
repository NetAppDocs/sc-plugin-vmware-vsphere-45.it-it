...
[role="lead"]
Prima di implementare l'appliance virtuale, è necessario conoscere i requisiti di implementazione. I requisiti di implementazione sono elencati nelle quattro tabelle seguenti.
...
* È necessario implementare il plug-in VMware di SnapCenter come macchina virtuale Linux.
+
...
| Destinazioni principali | Per eseguire la protezione basata su applicazioni su VMware SnapCenter standard per la protezione di macchine virtuali VMware e datastore solo SnapRestore: Utilizzato per le operazioni di ripristino FlexClone: Utilizzato per le operazioni di montaggio e collegamento 


| Destinazioni secondarie | Per eseguire la protezione basata sull'applicazione rispetto allo standard VMware SnapCenter: Utilizzato per le operazioni di failover per eseguire la protezione di macchine virtuali VMware e datastore solo FlexClone: Utilizzato per le operazioni di montaggio e collegamento 
|===
...
| VCenter vSphere | Il client HTML5: 6.5U2/U3, 6.7x, 7.0, 7.0U1, 7.0U2 Flex non è supportato. 
...
| Plug-in SnapCenter per database VMware vSphere MySQL | MySQL 8.0.23 


| VMware TLS | 1.2 


| TLS sul server SnapCenter | TLSv1.1 e versioni successive, il server SnapCenter utilizza questa funzione per comunicare con il plug-in VMware di SnapCenter per le operazioni di protezione dei dati VMDK. 
...
|===
Per informazioni aggiornate sulle versioni supportate, consultare https://mysupport.netapp.com/matrix/imt.jsp?components=91324;&solution=1517&isHWU&src=IMT["Tool di matrice di interoperabilità NetApp"^].
...
| Plug-in SnapCenter per porta VMware vSphere | 8144 (HTTPS), bidirezionale: La porta viene utilizzata per le comunicazioni dal client Web VMware vSphere e dal server SnapCenter. 8080 bidirezionale questa porta viene utilizzata per gestire l'appliance virtuale. Nota: Non è possibile modificare la configurazione della porta. 
